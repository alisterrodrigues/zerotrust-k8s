...
import (
	...
	"github.com/capstone/zerotrust-k8s/internal/controller"
...
func init() {
	...
	utilruntime.Must(zerotrustv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	// DEFENSE NOTE: In production the metrics endpoint should be secured (TLS + authn/authz via kube-rbac-proxy
	// or equivalent). For local development and capstone evaluation we default to plain HTTP so Prometheus
	// or curl can scrape without certificate provisioning.
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "8abe8c45.capstone.io",
	...
	if err := (&controller.ZeroTrustPolicyReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "ZeroTrustPolicy")
		os.Exit(1)
	}
